...
const getChatRoomByUuid = async (uuid: string) => {
  try {
    const { data } = await api.get<{ payload: { uuid: string } }>(
      `chat/chatroom/uuid/${uuid}`,
    );
...
const getChatRoomById = async (chatRoomIds: string) => {
  try {
    const { data } = await api.get<{ payload: GetChatRoomByIdDto }>(
      'chat/chatroom',
      {
...
const getChatRoomByUserId = async (user_id: number, product_id = '') => {
  try {
    const { data } = await api.get<{ payload: GetChatRoomByUserIdDto[] }>(
      `chat/chatroom/${user_id}`,
      {
        params: { product_id },
      },
    );
