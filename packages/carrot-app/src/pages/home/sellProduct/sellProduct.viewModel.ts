...
const useSellProductViewModel = () => {
  ...
  const [priceSuggest, setPriceSuggest] = useState(
    data ? data.priceSuggest : true,
  );
  ...
  const { data: hCodeData, isSuccess: getHCodeSuccess } = useQuery(
    [`location/${seller_location}/adm_cd`],
